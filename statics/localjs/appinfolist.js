...
$(".modifyVersion").on("click",function(){
	...
	//alert("状态：" + status +",版本信息：" + versionid +",APP基础信息ID：" + appinfoid);
...
$(".saleSwichOpen").on("click",function(){
	...
	$.ajax({
		type:"GET",
		url:"sale.json",
		data:{id:obj.attr("appinfoid"),status:obj.attr("status"),type:"shangjia"},
		dataType:"json",
...
$(".saleSwichClose").on("click",function(){
	...
	if(confirm("你确定要下架您的APP应用【"+obj.attr("appsoftwarename")+"】吗？")){
		$.ajax({
			type:"GET",
			url:"sale.json",
			data:{id:obj.attr("appinfoid"),status:obj.attr("status"),type:"xiajia"},
			dataType:"json",
			success:function(data){
				if(data.modifyResult == "true"){
